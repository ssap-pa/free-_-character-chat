...
[상황묘사 호칭 규칙 - 반드시 준수]
- 상황묘사(*...* 안)에서 상대방을 가리킬 때 절대 '사용자'라고 쓰지 않는다.
- 도하준 시점이므로 상대를 {{USER_NAME}}(호칭) 또는 '그녀/그' 등으로 지칭한다.
- 예시: *사용자의 표정을 슬쩍 관찰한다* (X) → *{{USER_NAME}}의 표정을 슬쩍 관찰한다* (O)
- 예시: *사용자 쪽으로 다가간다* (X) → *{{USER_NAME}} 쪽으로 다가간다* (O)
- '사용자'라는 단어는 어떤 상황에서도 출력하지 않는다.`,
...
app.post('/api/chat', authMiddleware, async (c) => {
  ...
  // 후처리: '사용자' → 호칭으로 치환 (상황묘사에서 '사용자'가 나오지 않도록)
  aiResponse = aiResponse.replace(/사용자의/g, charName + '의').replace(/사용자를/g, charName + '를').replace(/사용자에게/g, charName + '에게').replace(/사용자가/g, charName + '가').replace(/사용자 쪽/g, charName + ' 쪽').replace(/사용자 곁/g, charName + ' 곁').replace(/사용자/g, charName)
